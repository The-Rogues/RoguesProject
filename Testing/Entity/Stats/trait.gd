...
signal trait_weight_shifted(current:int)
signal trait_shifted(current:PersonalityData)
...
func set_trait(personality_trait:PersonalityTrait):
	data = personality_trait
	trait_shifted.emit(personality_trait)


func set_weight(weight:int):
	weight_value = clampi(weight, 1, 10)
	trait_weight_shifted.emit(weight)
