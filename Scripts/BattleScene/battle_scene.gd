...
@export var player_hand: CardHand
...
func initialize(battle_configuration:BattleSceneConfiguration):
	...
	# Connect signals
	player_hand.play_card.connect(_on_try_play_card)
...
func _on_try_play_card(card_data:CardData):
...
func _on_use_item(item_index:int):
	print("using ", items[item_index].name)
...
func _on_button_button_up() -> void:
	...
	action.damage = 20
	var action_info = battle_manager.create_action_info(null, [character_entity])
