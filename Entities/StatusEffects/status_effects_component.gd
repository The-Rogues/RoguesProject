...
const STATUS_EFFECT_UI = preload("res://Entities/StatusEffects/status_effect_icon.tscn")
...
func apply_projectile_effects(projectile_action: ProjectileAction) -> ProjectileAction:
	var action:ProjectileAction = projectile_action.duplicate(true)
	for instance in status_effects:
		action.impact_damage = instance.effect.modify_projectile_damage(
				action.impact_damage,
				instance
		)
		if instance.effect.id == "repeater":
			action.shots += instance.stack_count
	
	find_and_remove_status("repeater")
	find_and_remove_status("charge")
